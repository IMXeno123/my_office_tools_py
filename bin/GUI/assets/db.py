...
class MySqlDatabases():
    ...
    def all_data(self):
        with open(f"{self.filepath}/config/config.json", mode="r", encoding="UTF-8") as f:
    ...
    def insert_data(self, setting_data:dict):
        with open(f"{self.filepath}/config/config.json", mode="r", encoding="UTF-8") as f:
            ...
            if type(setting_data) == dict:
                ...
                with open(f"{self.filepath}/config/config.json", mode="w", encoding="UTF-8") as file:
                    json.dump(self.data, file)
            else:
                creat_log("[error] No data!")
                
    def creat_settings(self):
        if not Path(f"{self.filepath}/config/config.json").exists():
            Path(f"{self.filepath}/config/").mkdir()
            default_path = [{"path":f"{self.filepath}"},{"theme":"darkly"}]
            with open(f"{self.filepath}/config/config.json", mode="w", encoding="UTF-8") as f:
